...
void main(void)
{
	...
	while(1)
	{
		...
		if (sw1==0)
		{
			while(sw1==0);					// start on button release
//pls comment or uncomment the part needed

// unicast address setting===============================================================
// ==========================transmitter=================================================
				
			xb_MY(add_xb,0xABCD);			// set the source address, MY to ABCD
			xb_DL(add_xb,0x00001234);		// set the destination address, DL to 0x1234			
			xb_CH(add_xb, 0x1A);			// select a channel for transmission
//===========================receiver====================================================
			xb_MY(add_xb2,0x1234);			// set the source address, MY to 1234
			xb_DL(add_xb2,0x0000ABCD);		// set the destination address, DL to 0xABCD
			xb_CH(add_xb2, 0x1A);			// receiver need to be in the same channel 
		...
		if (sw2==0)
		{
			...
			while(sw2==0);					// start on button release
// broadcast address setting=============================================================
// ==========================transmitter=================================================
// transmitter address is set to broadcast address
			...
			xb_ID(add_xb,0x1234);			//	select a PAN ID
...
//===========================receiver====================================================
			...
			xb_CH(add_xb2, 0x0B);			// receiver need to be in the same channel 
											// to receive from transmitter
			xb_ID(add_xb2,0x1234);			// set PAN ID to be same
		...
		if (rec_data[0]=='I') led1=1;		//turn on LED if data received is correct
		if (rec_data[1]=='F') led2=1;
		if (rec_data[2]=='C') led3=1;
		if (rec_data[3]=='-') led4=1;
		if (rec_data[4]=='X') led5=1;
		if (rec_data[5]=='B') led6=1;
		if (rec_data[6]=='0') led7=1;
		if (rec_data[7]=='1') led8=1;
